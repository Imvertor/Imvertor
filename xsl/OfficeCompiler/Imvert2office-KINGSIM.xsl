...
<!-- 
    SVN: $Id: Imvert2office-KINGSIM.xsl 7397 2016-01-26 15:02:41Z arjan $ 
-->
<xsl:stylesheet 
    ...
    <!-- 
          Transform KING UML constructs to an office input document (HTML).
    -->
    ...
    <xsl:variable name="stylesheet">Imvert2office-KINGSIM</xsl:variable>
    <xsl:variable name="stylesheet-version">$Id: Imvert2office-KINGSIM.xsl 7397 2016-01-26 15:02:41Z arjan $</xsl:variable>
    ...
    <xsl:template match="imvert:package">
        ...
        <h1>Attribuut- en relatiesoorten</h1>
    ...
    <xsl:template match="imvert:class[imvert:stereotype = imf:get-config-stereotypes('stereotype-name-enumeration')]">
        ...
        <xsl:sequence select="imf:label-waarde($naam,$note)"/>
     </xsl:template>
    ...
    <xsl:template match="imvert:class[imvert:stereotype = imf:get-config-stereotypes('stereotype-name-enumeration')]" mode="detail">
        ...
        <table>
            <tbody>
                ...
                <td width="80%">
                    <xsl:sequence select="imf:get-clean-documentation-string(imvert:documentation)"/>
                </td>
            </tbody>
        </table>
        <table>
   ...
   <xsl:template match="imvert:attribute" mode="detail">
    ...
    <xsl:function name="imf:label-waarde" as="element()"> 
        ...
        <tr>
            <td width="30%"><b><xsl:value-of select="$label"/></b></td>
            <td width="70%"><xsl:value-of select="$waarde"/></td>
        </tr>
    </xsl:function>
