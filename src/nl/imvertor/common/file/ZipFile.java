...
public class ZipFile extends AnyFile {
    ...
    public void serializeToXml(AnyFolder serializeFolder) throws Exception {
    	...
    	contentWriter.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?><files>");
    	...
    	for (int i = 0; i < files.size(); i++) {
    		...
    		if (f.isDirectory())
    			...
    			if (fx.isWellFormed()) {
     				contentWriter.append("<file type=\"xml\" path=\"" + relpath + "\">");
    		    	int linesRead = 0;
    				while (true) {
			...
			if (!done) {	
				...
				contentWriter.append("<file type=\"bin\" path=\"" + relpath + "\"/>");
			}
    	}
    	contentWriter.append("</files>");
    	contentWriter.close();
    ...
    public void deserializeFromXml(AnyFolder serializeFolder, boolean replace) throws Exception {
    	...
    	List<Node> nodes = getElements(getElements(getNodes(dom.getChildNodes()),"files").get(0).getChildNodes(),"file"); // get all <file> nodes. 
